...
- (void)login
{
    ...
    [self.networkManager login:account password:password callback:^(AVUser * _Nullable user, NSError * _Nullable error) {
        ...
        if (error) {
            ...
            //NSDate转NSString
            NSString *createdAtString = [dateFormatter stringFromDate:user.createdAt];
            NSString *updateAtString = [dateFormatter stringFromDate:user.updatedAt];
