...
- (void)registerUser
{
    ...
    [self.networkManager checkCaptcha:captcha phoneNumber:phoneNumber callback:^(BOOL succeeded, NSError * _Nullable error) {
        ...
        if (succeeded) {
            [self.networkManager checkPhoneIsExist:phoneNumber callback:^(BOOL existed, NSError * _Nullable error) {
                [[YSLoadingHUD shareLoadingHUD] dismiss];
                if (!existed) {
...
- (void)phoneExist {
    [self showTipLabelWithText:@"手机号已存在"];
}
- (void)acquireCaptchaFailure
{
    [self showTipLabelWithText:@"验证码发送失败，请稍后再试"];
}
